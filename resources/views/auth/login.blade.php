...
<div class="position-absolute start-50 top-50 translate-middle text-center">
    <form method="POST" action="{{ route('login') }}">
        ...
        <p>
            <ons-input id="password" type="password" modifier="underbar" class="@error('password') is-invalid @enderror" name="password" placeholder="Password" required autocomplete="current-password"></ons-input>
            @error('password')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
